...
// ── Morpho fork test ──────────────────────────────────────────────────────────
//
// Same scenario as LiquidationForkTest but uses Morpho Blue as the flash-loan
// source instead of Balancer.
//
// Morpho Blue on Base (0xBBBBBbbBBb9cC5e90e3b3Af64bdAF62C37EEFFCb):
//   • Fee = 0%
//   • Callback: onMorphoFlashLoan(assets, data) — bot must approve(morpho, assets)
//     before the callback returns; Morpho then safeTransferFrom the bot.
//   • USDC liquidity: Morpho Blue markets hold significant USDC on Base.
//
// ─────────────────────────────────────────────────────────────────────────────

contract MorphoLiquidationForkTest is Test {
    ...
    // ── Main test: crash oracle, verify Morpho liquidity, execute via Morpho ──
    function test_detectAndLiquidateViaMorpho() public {

        // ── Step 1: Verify Morpho holds enough USDC for the flash loan ────────
        uint256 morphoUsdcBefore = IERC20Min(USDC).balanceOf(MORPHO);
        emit log_named_decimal_uint("Morpho USDC balance", morphoUsdcBefore, 6);
        assertGe(morphoUsdcBefore, 10_500e6, "Morpho must hold >=10 500 USDC");
        ...
        // ── Step 3: Execute liquidation via Morpho flash loan ─────────────────
        // Strategy:
        //   • triggerLiquidationWithMorpho(MORPHO, BAL_VAULT, params)
        //   • Morpho lends 10 500 USDC with 0% fee
        //   • liquidationCall → receive ~10 WETH (full collateral, HF < 0.95)
        //   • Swap WETH→USDC via WETH_USDC_V3 (Aerodrome CL V3 pool)
        //   • approve(MORPHO, 10_500e6) — Morpho pulls repayment (0% premium)
        //   • Surplus stays in the bot
        V2ArbBot.LiquidationParams memory p = V2ArbBot.LiquidationParams({
            user:             testUser,
            collateralAsset:  WETH,
            debtAsset:        USDC,
            debtToCover:      10_500e6,
            collateralPool:   address(0),     // collateral IS WETH — no swap needed
        ...
        bot.triggerLiquidationWithMorpho(MORPHO, BAL_VAULT, p);
        ...
        emit log_named_decimal_uint("Bot USDC profit (Morpho)", usdcProfit, 6);
        emit log_named_decimal_uint("Bot WETH profit (Morpho)", wethProfit, 18);

        // ── Step 4: Assertions ────────────────────────────────────────────────
        bool profitable = (usdcProfit > 0) || (wethProfit > 0);
        assertTrue(profitable, "Morpho liquidation should yield positive profit");

        // Expect at least $5 000 USDC — same economics as Balancer, 0% fee
        assertGt(usdcProfit, 5_000e6, "expected USDC profit > $5 000 via Morpho");
