...
impl PoolType {
    pub fn is_algebra(&self) -> bool {
        match self {
            PoolType::Algebra(_) => true,
            _ => false,
        }
    }

    pub fn is_uniswapv3cl(&self) -> bool {
        match self {
            PoolType::UniswapV3CL(_) => true,
            _ => false,
        }
    }

    /// Any concentrated-liquidity pool (Algebra or UniswapV3CL).
...
pub enum DexVariant {
    UniswapV2,
    UniswapV3,
    /// Solidly volatile pair (x*y=k) – same ABI as V2, kept for clarity.
    SolidlyVolatile,
    /// Solidly stable pair (x³y+xy³=k) – same swap ABI but different AMM math.
    SolidlyStable,
}
...
impl From<StringRecord> for Pool {
    fn from(record: StringRecord) -> Self {
        let version = match record.get(1).unwrap_or("2") {
            ...
            "5" => DexVariant::SolidlyVolatile,
            "4" => DexVariant::SolidlyStable,
    ...
    pub fn cache_row(&self) -> (String, i32, String, String, u8, u8, u32) {
        (
            ...
            match self.version {
                ...
                DexVariant::SolidlyStable => 4,
                DexVariant::SolidlyVolatile => 5,
            },
            format!("{:?}", self.token0),
            format!("{:?}", self.token1),
            self.decimals0,
...
pub async fn load_all_pools_from_v2(
    ...
    let cache_path = format!("cache/{cache_dir}/.cached-pools.csv");
